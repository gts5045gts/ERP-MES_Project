...
//			lotDTO.setLotId(lotId);// LOT ID 생성 및 저장
	...
	public void traceLot(ProceedingJoinPoint pjp, TrackLot trackLot, List<MaterialUsageDTO> usages) throws Throwable {
...
//			result = pjp.proceed();
			...
			if (obj != null) {
				...
				Object parentLotId = obj.getClass().getMethod("getLotId").invoke(obj);
				...
				if (parentLotId != null) {
					...
					MaterialUsageDTO usage1 = MaterialUsageDTO.builder()
											.parentLotId((String) parentLotId) // 원자재 LOT ID
											.build();
					usages.add(usage1);	
				}
