...
public class ApprService {
	...
	public ApprFullDTO getApprovalDetail(Long reqId) {
		...
		if (!results.isEmpty()) {
			...
			List<ApprFullDTO.ApprLineInfo> approvalLines = lineResults.stream().map(this::convertToApprLineInfo)
	...
	private List<ApprDTO> convertToApprDTOList(List<Object[]> results) {
		return results.stream().map(this::convertToApprDTO).collect(Collectors.toList());
	}
	...
	public void cancelApproval(Long reqId, String loginId) {
		// 결재 문서 조회
		Appr appr = apprRepository.findById(reqId).orElseThrow(() -> new IllegalArgumentException("결재를 찾을 수 없습니다."));
	...
	private List<ApprDTO> filterByStatus(List<ApprDTO> allDtoList, String status) {
		switch (status) {
		case "pending":
			return allDtoList.stream().filter(dto -> PENDING_STATUS.equals(dto.getStatusLabel()))
					.collect(Collectors.toList());
		case "completed":
			return allDtoList.stream().filter(
					dto -> APPROVED_STATUS.equals(dto.getStatusLabel()) || REJECTED_STATUS.equals(dto.getStatusLabel()))
					.collect(Collectors.toList());
		default:
	...
	private List<ApprDTO> filterByStatusAndUser(List<ApprDTO> allDtoList, String status, String userId) {
		...
		if (userId != null) {
			userFilteredList = allDtoList.stream().filter(dto -> userId.equals(dto.getEmpId()))
					.collect(Collectors.toList());
		...
		switch (status) {
		case "pending":
			return userFilteredList.stream().filter(dto -> PENDING_STATUS.equals(dto.getStatusLabel()))
					.collect(Collectors.toList());
		case "completed":
			return userFilteredList.stream().filter(
					dto -> APPROVED_STATUS.equals(dto.getStatusLabel()) || REJECTED_STATUS.equals(dto.getStatusLabel()))
					.collect(Collectors.toList());
		case "my":
...
//    	log.info("currentEmpId>>>>>>>>>>>>>>>>>>>>>>>>>"+currentEmpId);
		return apprRepository.findByNameContainingIgnoreCase(keyword, currentEmpId).stream()
				.map(PersonnelDTO::fromEntity).collect(Collectors.toList());
	}
	...
	public Long registAppr(@Valid ApprDTO apprDTO, String[] empIds, String loginEmpId) throws IOException {
		...
		// 인사발령은 ApprDetail에 관여하지 않기 때문에 null 체크를 통해 인사발령에서 이 부분을 건너뛰도록 if문 추가
