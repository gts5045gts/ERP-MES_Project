...
document.addEventListener('DOMContentLoaded', function() {
	...
	// 추가: 데이터를 정렬하는 함수
	    ...
	    // 추가: 테이블 헤더에 정렬 아이콘 클래스를 업데이트하는 함수
        ...
        dataToDisplay.forEach(personnel => {
            const row = `
                <tr>
                    ...
                    <td><button class="btn btn-primary btn-sm">상세조회</button></td>
                </tr>
            `;
    ...
    // 이 시점에 Thymeleaf는 이미 초기 데이터를 HTML에 채웠지만,
    // JS에서 필터링/페이지네이션을 위해 모든 데이터를 다시 가져와 관리합니다.
    fetchAllPersonnel();
});
